...
class MrpCostStructure(models.AbstractModel):
    ...
    def get_lines(self, productions):
        ...
        for product in productions.mapped('product_id'):
            ...
            # get the cost of service_charges effectively used
            service_charge_ids = []
            query_str = """SELECT sc.product_id, sc.bom_line_id, abs(SUM(sc.quantity)),abs(SUM(sc.subtotal))
                             FROM service_charge AS sc
                            WHERE sc.production_id in %s AND sc.bom_line_id is not null
                         GROUP BY sc.bom_line_id, sc.product_id"""

            self.env.cr.execute(query_str, (tuple(mos.ids),))
            for product_id, bom_line_id, qty,cost in self.env.cr.fetchall():
                service_charge_ids.append({
                    'qty': qty,
                    'cost': cost or 0,
                    'product_id': ProductProduct.browse(product_id),
                    'bom_line_id': bom_line_id
            ...
            # get the cost of service_charges effectively used not have bom line
            query_str = """SELECT sc.product_id, abs(SUM(sc.quantity)),abs(SUM(sc.subtotal))
                             FROM service_charge AS sc
                            WHERE sc.production_id in %s and sc.bom_line_id is null
                         GROUP BY sc.product_id"""

            self.env.cr.execute(query_str, (tuple(mos.ids),))
            for product_id, qty,cost in self.env.cr.fetchall():
                service_charge_ids.append({
                    'qty': qty,
                    'cost': cost or 0,
                    'product_id': ProductProduct.browse(product_id),
            ...
            res.append({
                ...
                'service_charge_ids' : service_charge_ids,
            })
        return res
